...
export default function FormFooterContact() {
    ...
    const [textAreaRowCount, setTextAreaRowCount] = useState<number>(1);
    ...
    return (
        <form className="w-full mt-1 footer-contact-form" method="POST" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="relative z-0 w-full mb-3 group">
                <textarea {...register("message")} className="footer-textarea block py-2.5 px-0 w-full text-sm text-white bg-transparent border-0 border-b-2 border-gray-600 appearance-none focus:border-green-500 focus:outline-none focus:ring-0 focus:border-green-600 peer" rows={textAreaRowCount} onFocus={() => setTextAreaRowCount(3)} onBlur={() => setTextAreaRowCount(1)} placeholder=" " required disabled={loadingState}></textarea>
