export default function FormComingSoonContact() {
    return <form className="w-full h-full" id="notify-me" method="POST" action="./includes/contact-form-handler.php">
        <div className="w-fit mx-auto">
            ...
            <div className="mt-8" style={{ display: "flex", alignContent: "center", verticalAlign: "middle", alignItems: "flex-start" }}>
                <input type="email" name="floating_email" id="floating_email" placeholder="Email Address" className="px-3 py-2.5 mx-0 text-lg text-white bg-gray-700 rounded-l-lg border-0 h-full focus:ring-0 focus:ring-offset-0 focus:outline-0 sm:w-48 md:w-60 lg:w-96 h-12" style={{ height: "48px" }} required />
                <button type="submit" className="g-recaptcha text-white bg-green-700 hover:bg-green-800 font-medium rounded-r-lg text-sm w-auto px-5 py-2.5 mx-0 text-center h-full border-0 focus:ring-0 origin-center -translate-x-1 -translate-y-px" data-sitekey="reCAPTCHA_site_key" data-callback="onSubmit" data-action="submit" style={{ height: "48px", marginTop: "1px" }}>
